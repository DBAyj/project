...
sleep 1
"$root/.venv/bin/python" "$root/scripts/p4_service_client.py" --socket "$socket" --token-file "$token_file" --health
ctest --test-dir "$root/runtime/tmp/p4-build" -R '^astra-shell-p4-projection-integration-tests$' --output-on-failure
"$root/.venv/bin/python" "$root/scripts/p4_service_client.py" --socket "$socket" --token-file "$token_file" --stop
