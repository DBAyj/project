...
nohup "$root/runtime/tmp/p4-build/services/astra-projection-service/astra-projection-service" --socket "$socket" \
  > "$root/runtime/logs/p4-projection-service.log" 2>&1 < /dev/null &
echo $! > "$service_pid_file"
for attempt in {1..30}; do
  if "$root/.venv/bin/python" "$root/scripts/p4_service_client.py" --socket "$socket" --health >/dev/null 2>&1; then break; fi
  if (( attempt == 30 )); then
...
"$root/.venv/bin/python" "$root/scripts/p4_service_client.py" --socket "$socket" --health >/dev/null
nohup env QSG_RHI_BACKEND=metal ASTRA_PROJECT_ROOT="$root" ASTRA_P4_PROJECTION_SOCKET="$socket" ASTRA_P4_DEMO_ON_START=1 \
