...
if [[ -S "$socket" && -f "$token_file" ]]; then "$root/.venv/bin/python" "$root/scripts/p4_service_client.py" --socket "$socket" --token-file "$token_file" --stop >/dev/null 2>&1 || true; fi
